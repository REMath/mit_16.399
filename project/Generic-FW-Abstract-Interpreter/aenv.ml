...
(* uninitialization *)
let initerr () = Array.create (number_of_variables ()) (Avalues.initerr ())
(* supremum *)
...
let set r x v = 
   if (Avalues.eq v (Avalues.bot ())) & (Avalues.isbotempty ()) then (* reduce *)
      (bot ())
   else
...
(* widening *)
let widen r1 r2 =
	let f i v = (Avalues.widen (get r1 i) v) in
		mapi f r2
(* narrowing *)
let narrow r1 r2 =
	 let f i v = (Avalues.narrow (get r1 i) v) in
		  mapi f r2
