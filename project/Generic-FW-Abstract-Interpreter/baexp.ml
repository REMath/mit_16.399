...
let rec b_aexp' a r p =
   ...
   | (MOD (a1, a2))  ->
     let (p1,p2) = (Avalues.b_MOD (Aaexp.a_aexp a1 r) (Aaexp.a_aexp a2 r) p)
       in (Aenv.meet (b_aexp' a1 r p1) (b_aexp' a2 r p2))
let do_trace_aexp a r p r' = if trace_aexp () 
   then ( print_string "Baexp["; print_Aexp a; print_string "]("; Aenv.print r; print_string ")<";  Avalues.print p; print_string "> = "; Aenv.print r'; print_newline (); r') 
   else r'
let b_aexp a r p =
   do_trace_aexp a r p (if (Aenv.is_bot r) && (Avalues.isbotempty ()) 
	                    then (Aenv.bot ()) else b_aexp' a r p)
